...
class BiliPlayerView @JvmOverloads constructor(
    ...
    init {
        ...
        if (isInEditMode.not()) {
            ...
            danmakuView.apply {
//                alpha = 0.4F
//                setBackgroundColor(ContextCompat.getColor(getContext(), Color.BLACK))
//                zOnTop = true
//                danmakuConfig.apply {
//                    textSizeCoeff = 1.8F
//                    typeface = Typeface.DEFAULT_BOLD
//                    lineHeight = (40F * 1.8F).roundToInt()
//                    drawMode = DanmakuConfig.DrawMode.STROKE
//                    stokeWidth = 1F
//                }
            }
            // 父视图40%高
            danmakuView.layoutParams = LinearLayout.LayoutParams(MATCH_PARENT, 0, 0.4F)
            val dmkLinearLayout = LinearLayout(context).apply {
                orientation = LinearLayout.VERTICAL
                weightSum = 1F
                addView(danmakuView)
            }
//            playerView.overlayFrameLayout!!.setBackgroundColor(Color.BLACK)
//            dmkLinearLayout.setBackgroundColor(Color.BLACK)
            playerView.overlayFrameLayout!!.addView(dmkLinearLayout, MATCH_PARENT, MATCH_PARENT)
//            playerView.overlayFrameLayout!!.addView(danmakuView, MATCH_PARENT, MATCH_PARENT)
    ...
    fun setCover(url: String?) {
        findViewById<SimpleDraweeView>(R.id.iv_cover).setImageURI(url)
    }
